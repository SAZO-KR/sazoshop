...
const ProductsTest: FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('Option1: ', prdInfo?.selectedAttribute(0));
    console.log('Option2: ', prdInfo?.selectedAttribute(1));
    console.log('Option3: ', prdInfo?.selectableAttributes(2));
  }, [prdInfo]);
  ...
  return (
    <Container sx={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <Container>
            ...
            {prdInfo.options.map((option, optionIdx) => (
              <Container key={optionIdx}>
                ...
                <Container>
                  {prdInfo
                    .selectableAttributes(optionIdx)
                    ?.map((attr, attrIdx) => (
                      <Button
                        key={attrIdx}
                        onClick={() => {
      ...
      <Button
        onClick={() => {
          prdInfo?.selectAttribute(0, 'BLUE');
